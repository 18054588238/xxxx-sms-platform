...
public class BlackGlobalFilterImpl implements ChainFilter {
    @Autowired
    ...
    public void check(StandardSubmit submit) {
        log.info("【策略模块-黑名单校验】平台级黑名单校验ing…………");
        String mobile = submit.getMobile();
        String result = cacheFeignClient.get(CacheConstant.MOBILE_BLACK + mobile);
        if (TRUE.equals(result)) {
            // 该手机号是黑名单
            log.info("【策略模块-黑名单校验】校验失败，该手机号是: 平台级黑名单,mobile:{}",mobile);
            // 校验失败，发送消息
        ...
        log.info("【策略模块-黑名单校验】平台级黑名单校验成功");
    }
}
